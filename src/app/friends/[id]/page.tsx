...
function FriendProfile() {
  ...
  function sendMsg(parent: { name: string; phone: string }, tplKey: keyof ReturnType<typeof getMsgTemplates>) {
    if (!profile) return;
  ...
  return (
    <div className="flex flex-col h-[100dvh] overflow-hidden bg-[#f7f6fb]">
      ...
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3">
        ...
        {/* הודעות לבירור פרטים */}
        {friend.parents.filter(p => p.phone).length > 0 && (
          <div className="bg-white rounded-xl border border-[#e0ddf0] overflow-hidden" dir="rtl">
            <p className="text-[11px] font-medium text-gray-400 px-4 pt-3 pb-1">שליחת הודעות לבירור פרטים</p>
            {friend.parents.filter(p => p.phone).map((p, pi) => (
              <div key={pi} className={pi > 0 ? 'border-t border-[#f0eef8]' : ''}>
                {friend.parents.filter(x => x.phone).length > 1 && (

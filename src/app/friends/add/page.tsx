...
function buildMsgVars(profile: UserProfile, friendChildName: string, parent: FriendParent) {
  return {
    parentFirst: parent.name.split(' ')[0],
    myName:      profile.parent.name.split(' ')[0],
    myChild:     getActiveChild(profile).name.split(' ')[0],
    friendName:  friendChildName,
...
function AddFriendForm() {
  ...
  const sendAndWait = (parent: typeof parents[0], tplKey: keyof MessageTemplates = 'initial') => {
    if (!profile) return;
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        ...
        {step === 2 && (
          <div>
            ...
            {/* ── שליחת הודעות לבירור פרטים ── */}
            {profile && parents.filter(p => p.phone.trim()).length > 0 && (
              <div className="mb-5 bg-white rounded-xl border border-[#e0ddf0] overflow-hidden" dir="rtl">
                <p className="text-[11px] font-medium text-gray-400 px-4 pt-3 pb-1">שלח הודעה — שמור וחכה לתשובה</p>
                {parents.filter(p => p.phone.trim()).map((p, idx) => (
                  <div key={idx} className={idx > 0 ? 'border-t border-[#f0eef8]' : ''}>
                    {parents.filter(x => x.phone.trim()).length > 1 && (
                      <p className="text-[10px] text-gray-400 px-4 pt-2">{p.name.split(' ')[0]}</p>
                    )}

                    {/* כפתור ראשי מודגש */}
                    <div className="px-3 pt-2.5 pb-1">
                      <button
                        onClick={() => sendAndWait(p, 'initial')}
                        className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-xl bg-[#25D366] text-white font-semibold text-[14px]"
                      >
                        ...
                        שלח הודעה ראשונית למילוי כל הפרטים
                      </button>
                    </div>
                    ...
                    <div className="px-3 pt-1.5 pb-2 flex flex-col gap-1.5">
                      {([
                        { key: 'initial'  as const, label: 'הודעה #1 — ימים פנויים' },
                        { key: 'initial2' as const, label: 'הודעה #2 — כתובת' },
                        { key: 'initial3' as const, label: 'הודעה #3 — פרטים נוספים' },
                      ]).map(({ key, label }) => (
                        <button key={key}
                          onClick={() => sendAndWait(p, key)}
                          className="flex items-center gap-2 w-full px-3 py-2 rounded-lg bg-gray-50 border border-gray-200 text-right"
                        >

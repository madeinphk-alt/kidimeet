...
export default function LuzPage() {
  ...
  function buildWaContact(m: PlannedMeetup): { phone: string; message: string } | null {
    const f = getFriendById(m.friendIds[0]);
    if (!f) return null;
    const parent = f.parents.find(p => p.phone.trim());
    if (!parent) return null;
    const timeStr = `${formatTime(m.timeFrom)}–${formatTime(m.timeTo)}`;
    const message = `היי ${parent.name.split(' ')[0]}, רק לאשר את הפגישה: ${m.title} ב-${m.date} ${timeStr}${m.location ? ` (${m.location})` : ''} 😊`;
    return { phone: formatWaPhone(parent.phone), message };
  }
      ...
      <div className={clsx(
        ...
        {!isPast && (
          <div className="flex gap-1.5 mt-2.5" dir="rtl">
            {/* הודעה */}
            {(() => {
              const contact = buildWaContact(m);
              return contact ? (
                <button
                  onClick={() => openWa(contact.phone, contact.message)}
        ...
        {upcoming.length === 0 && (
          <Link href="/meetups" className="block bg-[#f0fdf4] border border-[#bbf7d0] rounded-xl px-5 py-5 text-center active:opacity-80 mb-4" dir="rtl">
            <p className="text-green-900 text-[15px] font-semibold leading-relaxed">
              לילד שלכם יש פרופיל וכמה חברים זמינים?<br />לחצו על מפגשים והתאימו להם מפגש מתאים<br />ללוז הפנויות וכולם שמחים
            </p>
